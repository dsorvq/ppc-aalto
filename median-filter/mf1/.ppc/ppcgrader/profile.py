...
def generate_derived_statistics(stat: Dict[str, float]) -> ProfileData:
    ...
    if wallclock is None:
        return ProfileData()
    if wallclock < 100_000:
        return ProfileData(
            wallclock=Quantity(optional_div(wallclock, 1e9), Unit.Seconds))
    ...
    result.wallclock = Quantity(optional_div(wallclock, 1e9), Unit.Seconds)
...
def statistics_terminal(stat: ProfileData):
    ...
    if not stat.wallclock:
        return None
    if stat.wallclock < 0.01:
        return None
